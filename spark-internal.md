...
Next Utils.getDefaultPropertiesFile is called which looks at env value SPARK\__CONF_\_DIR or SPARK\_HOME to get the absolute path of **spark-defaults.conf**. The priority is given to one that has been passed via command line ,next to spark-default.conf and next to default enviornment variable. Also any keys not starting with "spark." is removed.
